<template>
  <section>
    ...
    <article>
      ...
      <div>
        ...
        <input
          ...
          v-model="password"
        />
        <p v-if="!isValidPwd && password.length < 6" class="error_message">
          6자 이상의 비밀번호를 입력해주세요.
        </p>
        <p
          v-else-if="!isValidPwd && password.length > 12"
          class="error_message"
        >
    ...
    <div class="login_btn">
      <button class="login_button" :disabled="!checkForm" @click="submitForm">
        LOG IN
      </button>
    </div>
